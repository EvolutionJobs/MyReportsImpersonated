...
#Region "Enumerations"
		...
		Public Enum LoginResult
			LOGIN_OK = 0
			LOGIN_USER_DOESNT_EXIST = -1
			LOGIN_USER_ACCOUNT_INACTIVE = -2
		End Enum
...
#End Region
		Public Shared Function GetDirectoryEntry() As DirectoryEntry
			...
			'' ************* this path workd !!!! ******************
			'' ************* this path workd !!!! ******************
			' Dim oRoot As DirectoryEntry = New DirectoryEntry("LDAP://EVOSVR01/DC=evolutionjobs,DC=local")
			' dn:CN=Sanjay Patel,OU=Admin Users,OU=Users,OU=Warrington,DC=evolutionjobs,DC=local
			...
			dirEntry.Path = "LDAP://EVOSVR01/CN=Users;DC=evolutionjobs,dc=local"
			dirEntry.Username = "EVOLUTIONJOBS\broadlook"
			dirEntry.Password = "bungle"
			Return dirEntry
		End Function
		...
		Public Function IsValidADLogin(ByVal loginName As String, ByVal givenName As String, ByVal surName As String) As Boolean
			...
			Catch ex As Exception
				Throw ex ' MessageBox.Show(ex.Message, "Active Directory Error", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1)
			End Try
		End Function
		...
		Private Function GetGroups(ByVal _path As String, ByVal username As String, ByVal password As String) As Collection
			...
			Catch ex As Exception
				If ex.GetType Is GetType(System.NullReferenceException) Then
					Throw ex ' MessageBox.Show("does not have a group", "No groups listed", Throw ex ' MessageBoxButtons.OK, MessageBoxIcon.Error)
				...
				Else
					Throw ex ' MessageBox.Show(ex.Message.ToString, "Search Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
				End If
			End Try
			Return Groups
		End Function
		...
		''' <returns></returns>
		Public Shared Function GetUsersinGroup(ByVal GroupName As String) As System.Data.DataSet
			...
			Dim dtUser As System.Data.DataTable = dsUsers.Tables.Add("Users")
			dtUser.Columns.Add("UserName")
			dtUser.Columns.Add("DisplayName")
			dtUser.Columns.Add("EMailAddress")

			'Create default row
			Dim drUser As System.Data.DataRow = dtUser.NewRow()
			drUser("UserName") = "0"
			...
			If Not searchResults Is Nothing Then
				...
				For l As Integer = 0 To n - 1
					...
					'set a new empty row
					Dim rwUser As System.Data.DataRow = dtUser.NewRow()
		...
		''' </summary>
		''' <returns></returns>
		Public Shared Function GetAllGroups() As System.Data.DataSet
			...
			'Create a new table object within the dataset
			Dim dtGroup As System.Data.DataTable = dsGroup.Tables.Add("Groups")
			...
			If searchResults.Count > 0 Then
				...
				For Each Result As SearchResult In searchResults
					'set a new empty row
					Dim drGroup As System.Data.DataRow = dtGroup.NewRow()
